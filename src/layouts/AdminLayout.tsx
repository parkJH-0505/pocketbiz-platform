...
const AdminLayout = () => {
  ...
  const menuItems = [
    { path: '/admin/kpi-library', label: 'KPI 라이브러리', icon: Library },
    { path: '/admin/weighting', label: '가중치 정책', icon: Weight },
    { path: '/admin/scoring', label: '스코어링 정책', icon: Calculator },
    { path: '/admin/programs', label: '프로그램 관리', icon: Briefcase },
    { path: '/admin/match-rules', label: '매칭 룰', icon: GitMerge },
    { path: '/admin/quality', label: '데이터 품질', icon: Shield },
        ...
        <div className="p-6 border-b border-neutral-gray">
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
            <div className={`${collapsed ? 'hidden' : 'block'}`}>
              ...
              <p className="text-sm text-neutral-lighter mt-1">시스템 관리</p>
            </div>
            <button
        ...
        <div className="bg-white border-b border-neutral-border px-8 py-4 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-neutral-dark">
              {menuItems.find(item => location.pathname === item.path)?.label || 'Admin'}
            </h2>
