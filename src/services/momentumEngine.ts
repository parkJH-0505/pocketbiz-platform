...
export class MomentumEngine {
   ...
   * 실용적 모멘텀 점수 계산 (간단 버전)
   */
  async calculateMomentum(): Promise<{
    score: number;
    trend: 'rising' | 'falling' | 'stable';
    velocity: number;
    acceleration: number;
    breakdown: {
      consistency: number;    // 꾸준함 (50%)
      activity: number;       // 활동량 (30%)
      performance: number;    // 성과 (20%)
    };
    insights: {
      message: string;
      suggestion?: string;
    ...
    const allDependencies = [
      ...CACHE_DEPENDENCIES.KPI_DATA,
      ...CACHE_DEPENDENCIES.TASK_DATA,
      ...CACHE_DEPENDENCIES.DOCUMENT_DATA,
      ...CACHE_DEPENDENCIES.SESSION_DATA,
      ...CACHE_DEPENDENCIES.GOAL_DATA
    ];

    const cached = momentumCache.get(CACHE_KEYS.MOMENTUM_SCORE, allDependencies);
    if (cached) {
      console.log('[MomentumEngine] Using cached momentum data');
      return cached;
    }

    console.log('[MomentumEngine] Calculating fresh momentum data');

    // 1. 꾸준함 점수 (50% 가중치) - 가장 중요
    const consistency = this.calculateConsistencySimple();

    // 2. 활동량 점수 (30% 가중치) - 실제 사용량
    const activity = this.calculateActivitySimple();

    // 3. 성과 점수 (20% 가중치) - KPI 등 실제 결과
    const performance = this.calculatePerformanceSimple();
    ...
    // 인사이트 생성
    const insights = this.generateBusinessInsights(score, { consistency, activity, performance });

    // 트렌드 계산 (이전 점수와 비교)
    const previousScore = this.getPreviousScore();
    const trend = score > previousScore + 5 ? 'rising' :
                  score < previousScore - 5 ? 'falling' : 'stable';

    // 속도와 가속도 계산
    const velocity = score - previousScore;
    const previousVelocity = this.getPreviousVelocity();
    const acceleration = velocity - previousVelocity;

    // 예측 생성
    const predictions = this.generatePredictions(score, velocity, acceleration);
    ...
    const result = {
      ...
      velocity,
      acceleration,
      breakdown: { consistency, activity, performance },
      insights: {
        ...insights,
        suggestions: [insights.suggestion].filter(Boolean),
        warnings: score < 40 ? ['비즈니스 건강도가 낮습니다. 즉시 개선이 필요합니다.'] : undefined
      },
      predictions,
    ...
    // 캐시에 저장
    momentumCache.set(CACHE_KEYS.MOMENTUM_SCORE, result, allDependencies);

    // 이전 점수 저장
    this.savePreviousScore(score);
    this.savePreviousVelocity(velocity);
